...
def fit_one_epoch(model_train, model, loss, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, genval, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        #                         'lr'        : get_lr(optimizer)})
        #     pbar.update(1)
    print('train_loss:', total_loss / (iteration + 1), 'acc:' , total_accuracy / (iteration + 1), 'lr:', get_lr(optimizer))
    if local_rank == 0:
        # pbar.close()
        print('Finish Train')
    ...
    for iteration, batch in enumerate(genval):
        ...
        #     pbar.update(1)
    print('test_loss:', val_loss / (iteration + 1), 'acc:' , val_total_accuracy / (iteration + 1))            
    if local_rank == 0:
        # pbar.close()
        print('Finish Validation')
        loss_history.append_loss(epoch + 1, total_loss / epoch_step, val_loss / epoch_step_val)
        print('Train Loss: %.3f || Test Loss: %.3f ' % (total_loss / epoch_step, val_loss / epoch_step_val))
        print('Train Acc: %.3f || Test Acc: %.3f ' % (total_accuracy / epoch_step, val_total_accuracy / epoch_step_val))
