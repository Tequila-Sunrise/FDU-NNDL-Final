...
def load_dataset(dataset_path, train_own_data, train_ratio):
    ...
    if train_own_data:
        ...
        for character in os.listdir(test_path):
            ...
            #   对每张图片进行遍历
            #-------------------------------------------------------------#
            character_path = os.path.join(test_path, character)
            for image in os.listdir(character_path):
                test_lines.append(os.path.join(character_path, image))
                test_labels.append(types)
            types += 1
    else:
        #-------------------------------------------------------------#
        #   Omniglot数据集，遍历大循环
    ...
    # random.seed(1)
    # shuffle_index = np.arange(len(test_lines), dtype=np.int32)
    # shuffle(shuffle_index)
    # random.seed(None)
    test_lines    = np.array(test_lines,dtype=np.object)
    test_labels   = np.array(test_labels)
    # test_lines    = test_lines[shuffle_index]
