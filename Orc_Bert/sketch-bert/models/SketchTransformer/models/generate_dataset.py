...
modes = ['train', 'test']  # ['train', 'valid', 'test']
offsets = {'train': {}, 'valid': {}, 'test': {}}
save_dir = 'Oracle/memmap'  # simple_memmap
load_file = {'full': open('Oracle/memmap_sum.txt', 'w')}
max_len = 0
...
with open(sum_path, 'r') as rf:
    for line in rf:
        ...
        for mode in modes:
            ...
            max_len = 250
            print(
                'Num Count: < {} :{}, total:{}'.format(
                    max_len, (len_record < max_len).sum(), len(len_record)
                )
            )
            # print(save_path)
            ...
            tmp_memmap = np.memmap(
                save_path, dtype=np.int16, mode='write', shape=stack_data.shape
            )
            tmp_memmap[:] = stack_data[:]
            tmp_memmap.flush()
        load_file['full'].write(
            '{}\t{}\n'.format(
                os.path.join(save_dir, '{}.dat'.format(cls_name)), tmp_num
            )
        )
#
pkl.dump(offsets, open('Oracle/offsets.npz', 'wb'))
